...
#        while other[i] in '0123456789':
        # Drop the 10 comparisons in favor of two.
        char_ord = ord(other[i])
        while char_ord >= 48 and char_ord <= 57:
            ours = other[i]
            i = i + 1
...
#            other = self.__class__(other)
    ...
    def __pow__(self, other, mod=0):
        ...
        If the exponent is not an integer, we will simply convert
        things to floats first.
        """
        if not isinstance(other, Decimal):
            other = self.__class__(other)
