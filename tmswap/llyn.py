...
import rete
from rete import AlphaMemory, VAR_PLACEHOLDER
...
#       self.collector = None # Object collecting evidence, if any 
        ...
        self.tms = None  # This formula is not tied to a TMS
        self._closureMode = ""
        self._closureAgenda = []
...
#        progress("searchable  %s, %s" %(self.statements, (pred, subj, obj))
    ...
    def add(self, subj, pred, obj, why=None):
        ...
        """
        s = self._buildStoredStatement(subj, pred, obj, why)
        if isinstance(s, int):
            if s == 0:
                return 0
            return s
...
### If this is the workingContext of a TMS, only believed statements can be added
        if self.tms:
            tmsNode = self.tms.getTriple(subj, pred, obj)
...
#       The above lines do not (removeStatement does not scale)
    ...
    def _addStatement(self, s):
        """Add a storedStatement

you should never be calling this
...
### for internal checks
        alphaMem = self._index.setdefault((pred, subj, obj), AlphaMemory())
        alphaMem.add(s)

        
        if isVariable(s, obj):
            obj = VAR_PLACEHOLDER
        if isVariable(s, subj):
            subj = VAR_PLACEHOLDER
        if isVariable(s, pred):
            pred = VAR_PLACEHOLDER
        ...
        alphaMem = self._index.setdefault((None, subj, None), AlphaMemory())
        alphaMem.add(s)

        alphaMem = self._index.setdefault((None, subj, obj), AlphaMemory())
        alphaMem.add(s)

        alphaMem = self._index.setdefault((pred, None, None), AlphaMemory())
        alphaMem.add(s)

        rein = self.newSymbol('http://dig.csail.mit.edu/2005/09/rein/network#requester')
        alphaMem = self._index.setdefault((pred, None, obj), AlphaMemory())
        alphaMem.add(s)

        alphaMem = self._index.setdefault((pred, subj, None), AlphaMemory())
        alphaMem.add(s)
